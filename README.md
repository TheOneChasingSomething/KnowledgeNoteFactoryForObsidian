...
3. The link `- [[Author - Title]]` is inserted **into the project note under `Ressources` > `LLM`** and **into the index note under `LLM`**, grouped after the last bullet by the same author (grouping distinguishes `Claude` from `ClaudeIA` via word boundaries). Missing headings are created at the end of the note; an existing link is never duplicated.
