...
**Google OAuth2 (v1.13).** The script needs YouTube Data API credentials, and offers three setup paths; the plugin covers two of them. Set **Google client secrets** to the path of your `client_secrets.json` and it is exported as `YT_DL_SECRETS` (POSIX-quoted, `~` expanded) before every run — the env-variable path. Alternatively, run the command **"store Google credentials (keyring)"** once: it executes the configurable setup command in a terminal to store the secrets in the system keyring (the default one-liner replicates the repo's documented method — `keyring.set_password("yt_playlist_dl", "client_secrets", …)` reading `client_secrets*.json` from the project folder; note the script's own runtime hint references a `yt_playlist_dl.store_secret` module that does not exist in the package) — the recommended path, after which the client secrets file can be deleted. A **"revoke Google credentials"** command runs the script's `revoke` subcommand. The OAuth consent browser flow itself is handled by the script on first download.

**Guided download (v1.15).** The command **"download (guided — project & index)"** opens a modal that prefills the URL from the clipboard, lets you pick the **type** (auto/video/article), and select the **project** and **knowledge-index** notes with `[[`-style suggestions. On submit it passes them to the script as CLI flags (`--project`, `--knowledge-index`, configurable names) together with Obsidian's absolute **vault path** (`--vault`, toggleable) — since Obsidian always knows its own location. Note names are POSIX-quoted. **This requires your Python script to accept those flags**: it currently reads project/knowledge-index from `config.toml` (prompting when empty) and has no `--vault` option, so add the matching CLI options (or blank the flag names in settings to omit them). See `Download Helper Dashboard.md` for a Dataview dashboard that lists the notes the script creates (identified by their `download` field), separates failed downloads (`download: KO`), and offers buttons that trigger these commands.

**Dependency bootstrap (v1.14).** The command **"install / check dependencies"** runs a configurable command that creates the virtualenv if missing and runs `pip install -e .` **only when the libraries are absent** — the detection imports third-party dependencies (`yt_dlp`, `keyring`, `typer`) rather than the local package, because the package imports from the repo directory even without installation. It is idempotent (does nothing when everything is already installed), so it is the one command to run after cloning the repo on a new machine.
