...
**"Set project status (Pending / Pause)" (v1.6).** The modal prefills the active note when it looks like a project (projects folder or `project` frontmatter block), otherwise offers the same `[[`-style field. Applying a status does two things: it sets `project.status` in the frontmatter through Obsidian's `processFrontMatter` API (formatting-safe), then moves the project's **bookmark** using the core Bookmarks plugin — `Pending` files it under the bookmark group **`4 - InProgress`**, `Pause` under **`3 - OnHold`** (both group names configurable). Group titles are matched **case-insensitively and recursively at any depth** of the bookmark tree (v1.8.1) — an existing `4 - InProgress` group, even nested inside a parent group, is reused rather than duplicated. On a status change the bookmark is removed from **every** group carrying the other status (and from duplicates of the target), then added exactly once; groups are only created at the root when nothing matches, and other bookmarks are left untouched. If the core Bookmarks plugin is disabled, the frontmatter is still updated and a notice says no bookmark was moved.
